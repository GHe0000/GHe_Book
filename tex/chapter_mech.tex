...
（2）若物体在地面系中存在恒定不变的加速度$\vec{a}$，则在一同相对地面加速的参考系中，等效重力加速度\footnote{此处为负号的原因可参考\fullref{s_gxl}}为$\vec{g^{\prime}} = \vec{g} - \vec{a}$
\end{defi}
...
我们不难发现，\textbf{如果题目要求从某定点过另一定点，且已知这两点高度差（或者经过这两点的速度的关系），求速度的最小值，那么均可以使用\fullref{d_ptydjbj}快速解决}，运用此定理还可以解决方法一解决不了的问题，如下
...
\subsection{质心与系统总动量}
质心还可用于求系统的总动量。由前文质心定义\eqref{zxdy}，两端求导，得到（$m_{tot}$为系统总质量\footnote{这里下标tot为total缩写}）
\begin{subequations}
\begin{align*}
...
\subsection{质心系\quad \dag \protect \footnote{带“\dag”号的章节在高考中用处较少，仅用于简便推导其他二级结论，可只阅读“定理部分”}}

若我们选取系统的质心为参考点建立参考系，则我们称此参考系为\textbf{质心系}（记为CM系\footnote{CM为Center of Mass的缩写，意为“质心”}）。

\begin{theo}[label=zxxsd]{质心系速度}{}
根据前文\fullref{zxdy}，对其两端对时间求导，可得
...
在质心系中，质心速度为$0$，则根据\fullref{zxyxtzdl}，系统总动量为$0$，即\textbf{质心系下系统的总动量为零}。写成矢量式为
...
在地面参考系中，质心速度为$\vec{v_c}$，第$i$个相对质心速度为$\vec{v_{ci}}$，根据前文速度牵连章节\eqref{s_sdql}，第$i$个质点相对地面速度为
...
\path (vci.south) ++ (2em,-2em) node[anchor=north] (scalep2){\highlight{blue}{根据\fullref{zxxxtdl}，此项恒等于$0$}};
...
我们可以用此定理很快地得到完全非弹性碰撞损失能量公式。若两球发生完全非弹性碰撞，记两球质量和初速分别为$m_1$、$v_1$，$m_2$、$v_2$。则根据\fullref{zxxsd}，可得
...
其中$\mu=\frac{m_1 m_2}{m_1 + m_2}$为约化质量（约化质量定义见\fullref{s_yhzl}），$v_{in}$为两球相对接近速度。
\end{theo}
...
其中$\mu=\frac{m_1 m_2}{m_1 + m_2}$为约化质量（约化质量定义见\fullref{s_yhzl}），接近速度$v_{in} = v_2 - v_1$
\end{theo}
...
根据前文定义，记$\mu = \frac{m_A m_B}{m_A + m_B}$为约化质量（约化质量定义见\fullref{s_yhzl}），因此上式可化简为
...
由\fullref{knxdl}可知，爆炸前由于两物体均没有相对质心运动，故系统动能为
...
其中$\mu = \frac{m_A m_B}{m_A + m_B}$为约化质量（约化质量定义见\fullref{s_yhzl}）。
\end{theo}
