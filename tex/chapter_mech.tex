...
\end{minipage}
~\\
右图涉及到换系下加速度牵连，请参考\secref{s_sdql}，图中$\vec{a}$为绝对加速度，$\vec{g^{\prime}}$为牵连加速度，$\vec{g}$为相对加速度。
\begin{defi}{等效重力加速度}{}

（1）若物体受到除重力外恒力$\vec{F}$，其对物体产生的加速度为$\vec{a_F} = \frac{\vec{F}}{m}$，则等效重力加速度为$\vec{g^{\prime}} = \vec{g} + \vec{a_F}$

（2）若物体在地面系中存在恒定不变的加速度$\vec{a}$，则在一同相对地面加速的参考系中，等效重力加速度\footnote{此处为负号的原因可参考\secref{s_gxl}}为$\vec{g^{\prime}} = \vec{g} - \vec{a}$
\end{defi}
...
根据前文\defiref{zxdy}，对其两端对时间求导，可得
...
\path (vci.south) ++ (2em,-2em) node[anchor=north] (scalep2){\highlight{blue}{根据\theoref{zxxxtdl}，此项恒等于$0$}};
...
我们可以用此定理很快地得到完全非弹性碰撞损失能量公式。若两球发生完全非弹性碰撞，记两球质量和初速分别为$m_1$、$v_1$，$m_2$、$v_2$。则根据\theoref{zxxsd}，可得
